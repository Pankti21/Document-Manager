...
const FileAnalyze = () => {
  ...
  React.useEffect(() => {
    ...
    Axios.get(`http://localhost:3001/analyze/${params.groupId}/${params.fileId}`)
      .then((response) => {
        setIsLoading(false);
    ...
    return (
      ...
      >
        <Alert variant="danger">
          ...
          <p>
            The request failed while analyzing your file. You can <a href={`/files/analyze/${params.fileId}`}>try again</a>.
          </p>
        </Alert>
      </div>
    );
  ...
  return (
    <div style={{ padding: "64px" }} className="container">
      <Image
        src={`http://localhost:3001/view/${params.groupId}/${params.fileId}`}
        style={{
          display: "block",
          maxHeight: "500px",
          margin: "32px auto",
      ...
      <Accordion defaultActiveKey="0">
        <AccordionItem eventKey="0">
          <AccordionHeader>
            Lines &middot;&nbsp; <b>{analysisResponse.lines.length}</b>
          </AccordionHeader>
          <AccordionBody>
            <ListGroup>
              {analysisResponse.lines.map((line) => {
                return <ListGroupItem>{line.text}</ListGroupItem>;
              })}
            </ListGroup>
          </AccordionBody>
        </AccordionItem>
        ...
        <AccordionItem eventKey="1">
          <AccordionHeader>
            Words &middot;&nbsp; <b>{analysisResponse.words.length}</b>
          </AccordionHeader>
          <AccordionBody>
            <ListGroup>
              {analysisResponse.words.map((word) => {
                return <ListGroupItem>{word.text}</ListGroupItem>;
              })}
            </ListGroup>
          </AccordionBody>
        </AccordionItem>
      ...
      <div style={{ display: "flex", justifyContent: "center", marginTop: "20px" }}>
        <Button type="submit" onClick={redirectHome}>
          Home
        </Button>
      </div>
    </div>
